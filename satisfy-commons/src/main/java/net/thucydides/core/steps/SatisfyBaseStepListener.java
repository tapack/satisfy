...
import static net.thucydides.core.steps.SatisfyBaseStepListener
        .ScreenshotType.MANDATORY_SCREENSHOT;
import static net.thucydides.core.steps.SatisfyBaseStepListener
        .ScreenshotType.OPTIONAL_SCREENSHOT;
 ...
 */
public class SatisfyBaseStepListener extends BaseStepListener {

    /**
     * Used to build the test outcome structure as the test step results come
     * in.
     */
    private final List<TestOutcome> testOutcomes;

    /**
     * Keeps track of what steps have been started but not finished, in order
     * to structure nested steps.
     */
    ...
    public Optional<TestStep> cloneCurrentStep() {
        return (Optional<TestStep>) ((currentStepExists()) ? getCurrentStep()
                .clone() : Optional.absent());
    }
    ...
    public SatisfyBaseStepListener(final File outputDirectory) {
     ...
     */
    public SatisfyBaseStepListener(final Class<? extends WebDriver>
                                           driverClass, final File
                                           outputDirectory) {
     ...
     * If the pages factory is null, a new driver will be created based on
     * the default system values.
     *
     * @param outputDirectory reports and screenshots are generated here
     * @param pages           a pages factory.
     */
    public SatisfyBaseStepListener(final File outputDirectory, final Pages
            pages) {
        this(outputDirectory);
        if (pages != null) {
     ...
     *
     * @param startedTestSuite the class implementing the test suite (e.g. a
     *                         JUnit test case)
     */
    ...
    public void testStarted(final String testMethod) {
        TestOutcome newTestOutcome = TestOutcome.forTestInStory(testMethod,
                testSuite, testedStory);
     ...
     * This step might be nested in another step, in which case the original
     * step becomes a group of steps.
     *
    ...
    private void recordStep(ExecutedStepDescription description) {
        String stepName = SatisfyAnnotatedStepDescription.from(description)
                .getName();

        updateFluentStepStatus(description, stepName);

        if (justStartedAFluentSequenceFor(description) ||
        ...
        inFluentStepSequence = SatisfyAnnotatedStepDescription.from
                (description).isFluent();
    }
    ...
    private void updateFluentStepStatus(ExecutedStepDescription description,
                                        String stepName) {
        if (currentlyInAFluentSequenceFor(description) ||
                justFinishedAFluentSequenceFor(description)) {
    ...
    private boolean justFinishedAFluentSequenceFor(ExecutedStepDescription
                                                           description) {
        boolean thisStepIsFluent = SatisfyAnnotatedStepDescription.from
                (description).isFluent();
        return (inFluentStepSequence && !thisStepIsFluent);
    }

    private boolean justStartedAFluentSequenceFor(ExecutedStepDescription
                                                          description) {
        boolean thisStepIsFluent = SatisfyAnnotatedStepDescription.from
                (description).isFluent();
        return (!inFluentStepSequence && thisStepIsFluent);
    ...
    private boolean currentlyInAFluentSequenceFor(ExecutedStepDescription
                                                          description) {
        boolean thisStepIsFluent = SatisfyAnnotatedStepDescription.from
                (description).isFluent();
        return (inFluentStepSequence && thisStepIsFluent);
    ...
    private void setDefaultResultFromAnnotations(final TestStep step, final
    ...
    private Optional<TestStep> getPreviousStep() {
        if (getCurrentTestOutcome().getTestSteps().size() > 1) {
            List<TestStep> currentTestSteps = getCurrentTestOutcome()
                    .getTestSteps();
            return Optional.of(currentTestSteps.get(currentTestSteps.size() -
                    2));
        } else {
    ...
    private TestStep getCurrentGroup() {
        if (currentGroupStack.isEmpty()) {
            ...
            return currentGroupStack.peek();// findLastChildIn
            // (currentGroupStack.peek());
        }
    }
...
//        markCurrentStepAs(SUCCESS);
    ...
    public void stepFailed(StepFailure failure) {
        ...
        getCurrentTestOutcome().determineTestFailureCause(failure
                .getException());
//        markCurrentStepAs(failureAnalysis.resultFor(failure));
    ...
    private void recordFailureDetailsInFailingTestStep(final StepFailure
                                                               failure) {
        if (currentStepExists()) {
            getCurrentStep().failedWith(new StepFailureException(failure
...
//        markCurrentStepAs(PENDING);
        ...
        if (currentStepExists() && browserIsOpen()) {
            try {
                ...
                if (screenshotAndHtmlSource.isPresent()) {
                    takeScreenshotIfRequired(screenshotType,
                            screenshotAndHtmlSource.get());
                }
    ...
    private void removeDuplicatedInitalScreenshotsIfPresent() {
        if (currentStepHasMoreThanOneScreenshot() && getPreviousStep()
                .isPresent() && getPreviousStep().get().hasScreenshots()) {
            ScreenshotAndHtmlSource lastScreenshotOfPreviousStep =
                    lastScreenshotOf(getPreviousStep().get());
            ScreenshotAndHtmlSource firstScreenshotOfThisStep =
                    getCurrentStep().getFirstScreenshot();
            if (firstScreenshotOfThisStep.hasIdenticalScreenshotsAs
                    (lastScreenshotOfPreviousStep)) {
    ...
    private ScreenshotAndHtmlSource lastScreenshotOf(TestStep testStep) {
        return testStep.getScreenshots().get(testStep.getScreenshots().size()
                - 1);
    }

    private void takeScreenshotIfRequired(ScreenshotType screenshotType,
                                          ScreenshotAndHtmlSource
                                                  screenshotAndHtmlSource) {
        if (shouldTakeScreenshot(screenshotType, screenshotAndHtmlSource) &&
                screenshotWasTaken(screenshotAndHtmlSource)) {
            getCurrentStep().addScreenshot(screenshotAndHtmlSource);
        }
    }

    private boolean screenshotWasTaken(ScreenshotAndHtmlSource
                                               screenshotAndHtmlSource) {
    ...
    private boolean shouldTakeOptionalScreenshot(ScreenshotAndHtmlSource
                                                         screenshotAndHtmlSource) {
        return (screenshotAndHtmlSource.wasTaken() && previousScreenshot()
                .isPresent()
                && (!screenshotAndHtmlSource.hasIdenticalScreenshotsAs
                (previousScreenshot().get())));
    }

    private Optional<ScreenshotAndHtmlSource> previousScreenshot() {
        List<ScreenshotAndHtmlSource> screenshotsToDate =
                getCurrentTestOutcome().getScreenshotAndHtmlSources();
        if (screenshotsToDate.isEmpty()) {
            return Optional.absent();
        } else {
            return Optional.of(screenshotsToDate.get(screenshotsToDate.size()
                    - 1));
        }
    }
    ...
    private void takeInitialScreenshot() {
        if ((currentStepExists()) && (screenshots().areAllowed
                (TakeScreenshots.BEFORE_AND_AFTER_EACH_STEP))) {
            take(OPTIONAL_SCREENSHOT);
        }
    }
    ...
    private Optional<ScreenshotAndHtmlSource> grabScreenshot() {
        ...
        if (screenshot.isPresent()) {
            if (shouldStoreSourcecode()) {
                File sourcecodeFile = sourcecodeForScreenshot(screenshot.get
                        (), getPageSource());
    ...
    private File sourcecodeForScreenshot(File screenshotFile, String
            pageSource) {
        File pageSourceFile = new File(screenshotFile.getAbsolutePath() + "" +
                ".html");
    ...
    public Photographer getPhotographer() {
        ...
        if (blurCheck.blurLevel().isPresent()) {
            return new Photographer(driver, outputDirectory, blurCheck
                    .blurLevel().get());
        } else {
    ...
    private Comparator<? super TestOutcome> byStartTimeAndName() {
        return new Comparator<TestOutcome>() {
            public int compare(TestOutcome testOutcome1, TestOutcome
                    testOutcome2) {
                String creationTimeAndName1 = testOutcome1.getStartTime()
                        .getMillis() + "_" + testOutcome1.getMethodName();
                String creationTimeAndName2 = testOutcome2.getStartTime()
                        .getMillis() + "_" + testOutcome2.getMethodName();
    ...
    public boolean aStepHasFailed() {
        return ((!getTestOutcomes().isEmpty()) &&
                (getCurrentTestOutcome().getResult() == TestResult.FAILURE ||
                        getCurrentTestOutcome().getResult() == TestResult
                                .ERROR));
    }
    ...
    /**
     * The current scenario is a data-driven scenario using test data from
     * the specified table.
     */
    ...
    public void exampleStarted(Map<String, String> data) {
        if (getCurrentTestOutcome().isDataDriven() && !getCurrentTestOutcome
                ().dataIsPredefined()) {
            getCurrentTestOutcome().addRow(data);
        }
        currentExample++;
        getEventBus().stepStarted(ExecutedStepDescription.withTitle
                (exampleTitle(currentExample, data)));
    }
