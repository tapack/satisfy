...
public class InboxMessagesInspectorIMAP implements InboxMessagesInspector {
    @Override
    public void verifyInboxContainsMatchingLetter(String sender, String subject, String body) throws IOException, MessagingException {
        List<MessageBean> matchingLetters = EmailHelper.getLettersThatMatchesParameters(sender, subject, body, EmailHelper.getInboxFolder(PropertyAccessor.getImapProperties()));
    ...
    public void verifyInboxContainsMatchingLetterWithBodyFromFile(String sender, String subject, String filePath) throws IOException, MessagingException {
        List<MessageBean> matchingLetters = EmailHelper.getLettersThatMatchesParameters(sender, subject, EmailHelper.readFileToStringByPathInResources(filePath), EmailHelper.getInboxFolder(PropertyAccessor.getImapProperties()));
        Assert.assertThat(matchingLetters, is(not(empty())));
    }

    @Override
    public boolean accept(String acceptanceParam) {
        return acceptanceParam.equalsIgnoreCase(ServerSetup.PROTOCOL_IMAP);
    }
}
